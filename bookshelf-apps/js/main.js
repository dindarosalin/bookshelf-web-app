        ...
        if (getByTitle.length == 0) {
            ...
            if (getByAuthor.length == 0) {
                ...
                if (getByYear.length == 0) {
                    alert(`Tidak ditemukan data ${searchValue.value}`)
                }else{
        ...
        if (book.isCompleted == false) {
            ...
            <article class="book_item">
                ...
                <div class="action">
                    ...
                    <button class="yellow" onclick="editBook('${book.id}')">Update</button>
            ...
            <article class="book_item">
                ...
                <div class="action">
                    <button class="green" onclick="unreadedBook('${book.id}')">Belum diBaca</button>
                    <button class="yellow" onclick="editBook('${book.id}')">Update</button>
                    <button class="red" onclick="deleteBook('${book.id}')">Delete</button>
                </div>
            </article>
...
function readedBook(id) {
    ...
    const bookDataDetail = bookData.find(a => a.id == id);
    const messageElement = document.getElementById("message");

    if (!bookDataDetail) {
        messageElement.innerText = "Data tidak ditemukan.";
        messageElement.style.display = "block"; 
        return;
    }

    const confirmation = confirm(`Apakah Anda yakin ingin menyelesaikan membaca buku "${bookDataDetail.title}"?`);
  
    if (confirmation == true) {
      const newBook = {
        id: bookDataDetail.id,
        title: bookDataDetail.title,
        author: bookDataDetail.author,
        year: bookDataDetail.year,
        isCompleted: true
      }
  
      const updatedBookData = bookData.filter(a => a.id != id);
      localStorage.setItem(localStorageKey, JSON.stringify(updatedBookData));
  ...
  function unreadedBook(id) {
    ...
    const confirmation = confirm(`Apakah Anda yakin ingin menandai buku "${bookDataDetail.title}" belum dibaca?`);
...
function deleteBook(id) {
    let confirmation = confirm("Menghapus data secara permanen?")
